...
pub fn create_post(post: Post) -> ExternResult<Record> {
    ...
    if let Some(base) = post.image_hash.clone() {
        create_link(base, post_hash.clone(), LinkTypes::PostToPosts, ())?;
    }
...
pub fn delete_post(original_post_hash: ActionHash) -> ExternResult<ActionHash> {
    ...
    if let Some(base_address) = post.image_hash.clone() {
        let links = get_links(
            GetLinksInputBuilder::try_new(base_address, LinkTypes::PostToPosts)?.build(),
        )?;
        for link in links {
    ...
    let links = get_links(
        GetLinksInputBuilder::try_new(record.action().author().clone(), LinkTypes::PostsByThread)?
            .build(),
    )?;
    for link in links {
        if let Some(hash) = link.target.into_action_hash() {
...
#[hdk_extern]
pub fn get_posts_for_post(post_hash: EntryHash) -> ExternResult<Vec<Link>> {
    get_links(GetLinksInputBuilder::try_new(post_hash, LinkTypes::PostToPosts)?.build())
}

#[hdk_extern]
pub fn get_deleted_posts_for_post(
    post_hash: EntryHash,
