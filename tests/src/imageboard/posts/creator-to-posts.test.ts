...
test("link a Creator to a Post", async () => {
  await runScenario(async scenario => {
    ...
    // Alice creates a link from Creator to Post
    await alice.cells[0].callZome({
      zome_name: "posts",
      fn_name: "add_post_for_creator",
      payload: {
        base_creator: baseAddress,
    ...
    await alice.cells[0].callZome({
      zome_name: "posts",
      fn_name: "delete_post_for_creator",
      payload: {
        base_creator: baseAddress,
        target_post_hash: targetAddress,
      },
    ...
    let deletedLinksOutput: Array<[SignedActionHashed<CreateLink>, SignedActionHashed<DeleteLink>[]]> = await bob
      .cells[0].callZome({
        zome_name: "posts",
        fn_name: "get_deleted_posts_for_creator",
        payload: baseAddress,
      });
    ...
    deletedLinksOutput = await bob.cells[0].callZome({
      zome_name: "posts",
      fn_name: "get_deleted_creators_for_post",
      payload: targetAddress,
    });
    assert.equal(deletedLinksOutput.length, 1);
  });
